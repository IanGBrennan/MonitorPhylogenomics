...
gtree <- read.nexus("~/Documents/GitHub/MonitorPhylogenomics/Scripts_Files/Varanidae_Fossil.tre");
plot.phylo(gtree)
alldata <- read.csv("~/Documents/GitHub/MonitorPhylogenomics/Trait_Evo/All_Size_Data.csv", header=T)
head(alldata)
...
gf <- filter(goanna, is.na(Habitat)==FALSE, is.na(Tail)==FALSE, is.na(SVL)==FALSE);
```
...
```{r, eval=T, fig.height=10}
obj1 <- contMap(gf$phy, treeplyr::getVector(gf, logSVL), plot=FALSE, outline=F); 
n<-length(obj1$cols); 
...
Plot the distribution of habitat usage

```{r, eval=T, fig.height=10}
habitat.data <- treeplyr::getVector(gf, Habitat)
testo <- make.simmap(gf$phy, habitat.data)
cols <- brewer.pal(4, "Paired"); 
names(cols) <- c("saxicolous", 
                 "semiaquatic", 
                 "terrestrial", 
                 "arboreal")
plot(testo, cols)
...
```{r, message=F, warning=F}
source("~/Documents/GitHub/MonitorPhylogenomics/Scripts_Files/RPANDA_extras.R")
```
These functions make a *Geo Object* from spatial data and your *rase* output.  
```{r, message=F, warning=F}
source("~/Documents/GitHub/MonitorPhylogenomics/Scripts_Files/CreateGeoObject_fromSP.R")
```
A collection of functions for extracting the AIC values and weights for a set of models.  
```{r, message=F, warning=F}
source("~/Documents/GitHub/MonitorPhylogenomics/Scripts_Files/Calculate_AICs.R")
```
...
```{r, message=F, warning=F}
source("~/Documents/GitHub/MonitorPhylogenomics/Scripts_Files/plot.distmaps.R")
```
This function processes a *rase* output object and makes distribution objects for ancestral nodes. More on that later...  
```{r, message=F, warning=F}
source("~/Documents/GitHub/MonitorPhylogenomics/Scripts_Files/process.rase.R")
```
...
```{r, message=F, warning=F}
source("~/Documents/GitHub/MonitorPhylogenomics/Scripts_Files/search.surface.R")
```
...
```{r, eval=T, echo=F}
load("Varanus_Empirical_ModelObjects.RData")
```
...
library(OUwie)
source("~/Documents/GitHub/MonitorPhylogenomics/Scripts_Files/make.OUwie.input_Script.R")
...
```{r}
coevo <- readRDS("~/Documents/GitHub/MonitorPhylogenomics/Trait_Evo/CoEvo_Walkthrough.RDS")
names(coevo)
```
...
```{r, eval=T, echo=F}
load("Varanus.Marsupial_Empirical_ModelObjects.RData")
```
